...
class GraphDetector:
    def __init__(self, graph, nodes, labels, mask, feature, train_config: dict):
        ...
        select_anomaly = ((labels == 1) & (mask == 1)).to(torch.bool)
        select_normal = ((labels == 0) & (mask == 1)).to(torch.bool)
        ...
        self.model = xgb.XGBClassifier(n_estimators=n_estimators, learning_rate=learning_rate,
                                       eval_metric=roc_auc_score, max_depth=max_depth)
    ...
    def train(self, train_idx, train_y, test_idx):
        self.model.fit(self.feature[train_idx].detach().numpy(), train_y)
